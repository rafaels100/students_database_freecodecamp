...
PSQL="psql -X --username=root --dbname=students --no-align --tuples-only -h 127.0.0.1 -c"
...
echo "$($PSQL "SELECT course FROM students LEFT JOIN majors_courses USING(major_id) LEFT JOIN courses USING(course_id) GROUP BY course HAVING COUNT(*) = 1 ORDER BY course")"
